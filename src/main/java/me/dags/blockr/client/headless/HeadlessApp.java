...
import me.dags.blockr.App;
import me.dags.blockr.Flags;
import me.dags.blockr.Options;
...
public class HeadlessApp implements App {
    ...
    private void checkWorld(File file) {
        if (file.exists()) {
            if (!new File(file, "level.dat").exists()) {
                throw new IllegalArgumentException("The provided world dir does not contain a level.dat file");
            }
        } else {
            throw new IllegalArgumentException("The provided world dir does not exist: " + file);
        }
    }
    ...
    private void printHelp() {
        ...
        System.out.println("--world 'dir/path'   - the world directory");
